...
class AliasesTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->user->defaultUsername->username = 'mrunknown';
    ...
    {
        $response = $this->json('POST', '/api/v1/aliases', [
            'domain' => $this->user->username.'.mailflusher.com',
            'format' => 'custom',
            'description' => 'the description',
        ...
        $this->assertEquals('valid-local-part@'.$this->user->username.'.mailflusher.com', $this->user->aliases[0]->email);
    }
    ...
    public function user_cannot_generate_custom_alias_that_already_exists()
    {
        Alias::factory()->create([
            ...
            'extension' => '1',
            'domain' => $this->user->username.'.mailflusher.com',
            'email' => 'exists@'.$this->user->username.'.mailflusher.com',
        ]);
    ...
    public function user_can_update_alias_from_name()
    {
        ...
        $response = $this->json('PATCH', '/api/v1/aliases/'.$alias->id, [
            'from_name' => 'Mr Unknown',
        ]);

        $response->assertStatus(200);
        $this->assertEquals('Mr Unknown', $response->getData()->data->from_name);
    }
