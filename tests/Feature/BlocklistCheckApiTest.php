...
class BlocklistCheckApiTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        config(['mailflusher.blocklist.allowed_ips' => ['127.0.0.1'], 'mailflusher.blocklist.secret' => '']);
    }

    #[Test]
    public function returns_401_when_secret_required_and_missing(): void
    {
        config(['mailflusher.blocklist.secret' => 'shared-secret']);
        config(['mailflusher.blocklist.allowed_ips' => ['127.0.0.1']]);

        $response = $this->getJson('/api/blocklist-check?recipient=test@mrunknown.'.config('mailflusher.domain').'&from_email=spam@example.com');
    ...
    {
        config(['mailflusher.blocklist.allowed_ips' => ['10.0.0.1']]);

        $response = $this->withServerVariables(['REMOTE_ADDR' => '192.168.1.1'])
            ->getJson('/api/blocklist-check?recipient=test@mrunknown.'.config('mailflusher.domain').'&from_email=spam@example.com');
    ...
    public function returns_block_false_when_not_blocked(): void
    {
        ...
        $response = $this->getJson('/api/blocklist-check?recipient=ebay@mrunknown.'.config('mailflusher.domain').'&from_email=newsletter@example.com');
    ...
    public function returns_block_true_when_email_blocked(): void
    {
        ...
        $response = $this->getJson('/api/blocklist-check?recipient=ebay@mrunknown.'.config('mailflusher.domain').'&from_email=spam@example.com');
    ...
    public function returns_block_true_when_domain_blocked(): void
    {
        ...
        $response = $this->getJson('/api/blocklist-check?recipient=ebay@mrunknown.'.config('mailflusher.domain').'&from_email=news@spammer.com');
    ...
    public function increments_blocked_count_and_last_blocked_on_blocked_sender(): void
    {
        ...
        $this->getJson('/api/blocklist-check?recipient=ebay@mrunknown.'.config('mailflusher.domain').'&from_email=spam@example.com')
    ...
    public function increments_blocked_count_on_domain_blocked_sender(): void
    {
        ...
        $this->getJson('/api/blocklist-check?recipient=ebay@mrunknown.'.config('mailflusher.domain').'&from_email=news@spammer.com')
            ->assertJson(['block' => true]);
    ...
    {
        config(['mailflusher.blocklist.secret' => 'shared-secret']);
        config(['mailflusher.blocklist.allowed_ips' => []]);
