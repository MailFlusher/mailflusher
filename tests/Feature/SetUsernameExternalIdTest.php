...
class SetUsernameExternalIdTest extends TestCase
{
    ...
    public function handle_validation_should_fail_when_username_not_provided()
    {
        ...
        $this
            ->artisan('mailflusher:set-username-externalid')
            ->assertExitCode(1);
    }
    ...
    public function handle_validation_should_fail_when_external_id_not_provided()
    {
        $this->expectException(RuntimeException::class);
        $this
            ->artisan('mailflusher:set-username-externalid test_username')
            ->assertExitCode(1);
    }
    ...
    {
        $this
            ->artisan('mailflusher:set-username-externalid test_username test_external_id')
            ->assertExitCode(1);
    }
    ...
    {
        $this
            ->artisan('mailflusher:set-username-externalid mrunknown janedoe_ext_id')
            ->assertExitCode(1);
    }
    ...
    {
        $this
            ->artisan('mailflusher:set-username-externalid mrunknown mrunknown_ext_id')
            ->assertExitCode(0);

        $username = Username::where('username', 'mrunknown')->first();

        $this->assertThat($username->external_id, $this->equalTo('mrunknown_ext_id'), 'Username has externalId');
    }
    ...
    {
        $this->userMrUnknown->usernames()->create(['username' => 'test', 'can_login' => false]);

        $this
            ->artisan('mailflusher:set-username-externalid test mrunknown_ext_id')
            ->assertExitCode(0);

        $user = User::where('id', $this->userMrUnknown->id)->first();
