    ...
    public function build()
    {
        ...
        if ($this->alias->isCustomDomain()) {
            if (! $this->alias->aliasable->isVerifiedForSending()) {
                ...
                $this->verpDomain = config('mailflusher.domain');
            }
        }
        ...
        $this->email = $this
            ...
            ->withSymfonyMessage(function (Email $message) {
                ...
                $message->getHeaders()
                    ->addTextHeader('Feedback-ID', 'F:'.$this->alias->id.':mailflusher');
                ...
                if ($this->authenticationResults) {
                    $message->getHeaders()
                        ->addTextHeader('X-MailFlusher-Authentication-Results', $this->authenticationResults);
                }

                $message->getHeaders()
                    ->addTextHeader('X-MailFlusher-Original-Sender', $this->sender);

                $message->getHeaders()
                    ->addTextHeader('X-MailFlusher-Original-Envelope-From', $this->originalEnvelopeFrom);

                if ($this->originalFromHeader) {
                    $message->getHeaders()
                        ->addTextHeader('X-MailFlusher-Original-From-Header', base64_decode($this->originalFromHeader));
                }

                if ($this->originalReplyToHeader) {
                    $message->getHeaders()
                        ->addTextHeader('X-MailFlusher-Original-Reply-To-Header', base64_decode($this->originalReplyToHeader));
                }
                ...
                if ($this->originalCc) {
                    $message->getHeaders()
                        ->addTextHeader('X-MailFlusher-Original-Cc', $this->originalCc);
                }

                if ($this->originalTo) {
                    $message->getHeaders()
                        ->addTextHeader('X-MailFlusher-Original-To', $this->originalTo);
                }

                if ($this->isSpam) {
                    $message->getHeaders()
                        ->addTextHeader('X-MailFlusher-Spam', 'Yes');
                }

                if ($this->resend) {
                    $message->getHeaders()
                        ->addTextHeader('X-MailFlusher-Resend', 'Yes');
                }
            });
        ...
        if ($this->emailHtml) {
            ...
            // Optional tracker stripping, gated per-user
            $mode = $this->user->strip_trackers ?? 'off';

            // Downgrade to pixels_only if the user's plan no longer allows link stripping
            if ($mode === 'pixels_and_links' && ! $this->user->canUseLinkStripping()) {
                $mode = 'pixels_only';
            }

            if ($mode !== 'off') {
                try {
                    $stripper = app(TrackerStripper::class);
                    $html = $stripper->stripPixels($html);
                    if ($mode === 'pixels_and_links') {
                        $html = $stripper->proxyLinks($html, $this->alias);
                    }
                } catch (\Throwable $e) {
                    // Never let the stripper break an email
                    \Log::warning('TrackerStripper failed', ['alias_id' => $this->alias->id, 'error' => $e->getMessage()]);
                }
            }
    ...
    private function getUserDisplayFrom($displayFrom)
    {
        ...
        if ($displayFrom === $this->sender) {
            return match ($this->user->display_from_format) {
                ...
                default => Str::beforeLast($this->sender, '@').' - '.Str::afterLast($this->sender, '@'),
            };
        }

        // Check user display_from_format settings and then return correct format
        return match ($this->user->display_from_format) {
            DisplayFromFormat::DEFAULT => $displayFrom.' - '.Str::afterLast($this->sender, '@'),
            ...
            default => $displayFrom.' - '.Str::afterLast($this->sender, '@'),
        };
    }
