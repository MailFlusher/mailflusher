...
class ShowDashboardController extends Controller
{
    public function index()
    {
        ...
        $pendingLeakEvents = AliasLeakEvent::whereIn('alias_id', user()->aliases()->pluck('id'))
            ->where('status', 'pending')
            ->orderByDesc('detected_at')
            ->limit(5)
            ->get(['id', 'alias_id', 'sender_domain', 'detected_at']);
        ...
        return Inertia::render('Dashboard/Index', [
            ...
            'pendingLeakEvents' => $pendingLeakEvents,
            'pendingLeakCount' => AliasLeakEvent::whereIn('alias_id', user()->aliases()->pluck('id'))
                ->where('status', 'pending')
                ->count(),
        ]);
    }
