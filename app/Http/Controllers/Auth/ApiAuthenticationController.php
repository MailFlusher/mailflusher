...
use App\Http\Requests\Auth\ApiAuthenticationLoginRequest;
use App\Http\Requests\Auth\ApiAuthenticationMfaRequest;
use App\Http\Requests\Auth\DestroyAccountRequest;
...
class ApiAuthenticationController extends Controller
{
    ...
    public function login(ApiAuthenticationLoginRequest $request)
    {
        ...
        if ($user->two_factor_enabled) {
            return response()->json([
                ...
                'mfa_key' => Crypt::encryptString($user->id.'|'.config('mailflusher.secret').'|'.Carbon::now()->addMinutes(5)->getTimestamp()),
    ...
    public function mfa(ApiAuthenticationMfaRequest $request)
    {
        ...
        if (! $user || $parts[1] !== config('mailflusher.secret')) {
