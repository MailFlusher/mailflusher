...
use App\Http\Requests\Alias\GeneralAliasBulkRequest;
use App\Http\Requests\Alias\RecipientsAliasBulkRequest;
...
class AliasBulkController extends Controller
{
    ...
    public function forget(GeneralAliasBulkRequest $request)
    {
        ...
        $forgottenSharedDomainCount = user()->aliases()->withTrashed()
            ->whereIn('id', $aliasIds)
            ->whereIn('domain', config('mailflusher.all_domains'))
            ->update([
        ...
        if ($forgottenSharedDomainCount < $aliasIdsCount) {
            ...
            user()->aliases()->withTrashed()
                ->whereIn('id', $aliasIds)
                ->whereNotIn('domain', config('mailflusher.all_domains'))
                ->forceDelete();
        }
    ...
    public function restore(GeneralAliasBulkRequest $request)
    {
        ...
        $currentCount = user()->aliases()->count();

        if ($limit > 0 && $currentCount >= $limit) {
            return response()->json(['message' => 'You have reached your alias limit for your current plan. Please upgrade to restore more aliases.'], 403);
        }
        ...
        // Only restore up to the remaining slots
        if ($limit > 0) {
            $remaining = $limit - $currentCount;

            if ($remaining <= 0) {
                return response()->json(['message' => 'You have reached your alias limit for your current plan.'], 403);
            }

            $aliasIds = $aliasIds->take($remaining);
            $aliasIdsCount = $aliasIds->count();
        }

        // Use update since delete() does not trigger model event
        user()->aliases()->onlyTrashed()->whereIn('id', $aliasIds)->update(['active' => true, 'deleted_at' => null]);

        $skipped = count($request->ids) - $aliasIdsCount;

        // Don't return 204 as that is only for empty responses
        return response()->json([
            'message' => ($aliasIdsCount === 1 ? '1 alias restored successfully' : "{$aliasIdsCount} aliases restored successfully")
                .($skipped > 0 ? ". {$skipped} skipped due to plan limit." : ''),
            'ids' => $aliasIds,
        ], 200);
    }
    ...
    /**
     * Move a set of aliases into a group (or out of a group when alias_group_id is null).
     */
    public function group(GeneralAliasBulkRequest $request)
    {
        $request->validate([
            'alias_group_id' => [
                'nullable',
        ...
        $groupId = $request->input('alias_group_id');

        if ($groupId !== null) {
            // Ensure the group belongs to the current user
            $exists = \App\Models\AliasGroup::where('user_id', user()->id)->where('id', $groupId)->exists();
            if (! $exists) {
                return response()->json(['message' => 'Alias group not found'], 404);
            }
        }

        $aliases = user()->aliases()->whereIn('id', $request->ids);
        $count = $aliases->count();
        ...
        $aliases->update(['alias_group_id' => $groupId]);

        return response()->json([
            'message' => $count === 1 ? '1 alias moved successfully' : "{$count} aliases moved successfully",
            'ids' => $request->ids,
        ]);
    }
}
