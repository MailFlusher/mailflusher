...
use App\Http\Requests\Alias\IndexAliasRequest;
use App\Http\Requests\Alias\StoreAliasRequest;
use App\Http\Requests\Alias\UpdateAliasRequest;
...
class AliasController extends Controller
{
    ...
    public function store(StoreAliasRequest $request)
    {
        if (user()->hasReachedAliasLimit()) {
            return response('You have reached your alias limit for your current plan. Please upgrade to create more aliases.', 403);
        }
        ...
        $isBurner = $request->filled('expires_at') || $request->filled('max_emails');

        if ($isBurner && user()->hasReachedBurnerAliasLimit()) {
            return response('You have reached your active burner alias limit for your current plan. Please upgrade or deactivate existing burners.', 403);
        }
        ...
        // This is an mailflusher.com domain.
        if ($parentDomain) {
        ...
        if ($request->filled('max_emails')) {
            $data['max_emails'] = $request->max_emails;
        }

        if ($request->filled('on_expiry')) {
            $data['on_expiry'] = $request->on_expiry;
        }

        if ($request->boolean('ghost_mode')) {
            if (! user()->canUseGhostInbox()) {
                return response('Ghost Inbox is available on the Pro plan only.', 403);
            }
            if (! user()->hasGhostVault()) {
                return response('Set up your ghost inbox vault before enabling ghost mode.', 422);
            }
            $data['ghost_mode'] = true;
        }

        if ($request->filled('alias_group_id')) {
            $data['alias_group_id'] = $request->alias_group_id;
        }
    ...
    public function update(UpdateAliasRequest $request, $id)
    {
        ...
        if ($request->has('alias_group_id')) {
            $alias->alias_group_id = $request->alias_group_id;
        }
    ...
    {
        if (user()->hasReachedAliasLimit()) {
            return response('You have reached your alias limit for your current plan. Please upgrade to restore more aliases.', 403);
        }
