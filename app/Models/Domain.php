...
class Domain extends Model
{
    ...
    public function checkVerification()
    {
        ...
        try {
            return collect(dns_get_record($this->domain.'.', DNS_TXT))
                ->contains(function ($r) {
                    return trim($r['txt']) === 'mf-verify='.sha1(config('mailflusher.secret').user()->id.user()->domains->count());
                });
        } catch (Exception $e) {
    ...
    public function checkMxRecords()
    {
        ...
        if ($mx['target'] !== config('mailflusher.hostname')) {
            return false;
        }
    ...
    public function checkVerificationForSending()
    {
        ...
        try {
            $spf = collect(dns_get_record($this->domain.'.', DNS_TXT))
                ->contains(function ($r) {
                    return preg_match("/^(v=spf1).*(include:spf\.".config('mailflusher.domain').'|mx).*(-|~)all$/', $r['txt']);
                });
        } catch (Exception $e) {
        ...
        try {
            $dkim = collect(dns_get_record(config('mailflusher.dkim_selector').'._domainkey.'.$this->domain.'.', DNS_CNAME))
                ->contains(function ($r) {
                    return $r['target'] === config('mailflusher.dkim_selector').'._domainkey.'.config('mailflusher.domain');
                });
        } catch (Exception $e) {
        ...
        if (! $dkim) {
            return response()->json([
                'success' => false,
                'message' => 'CNAME '.config('mailflusher.dkim_selector').'._domainkey record not found. This could be due to DNS caching, please try again later.',
                'data' => new DomainResource($this->fresh()),
            ]);
        }
