...
class EmailData
{
    ...
    public function __construct(Parser $parser, $sender, $size, $emailType = 'F', $resend = false)
    {
        ...
        if ($originalCc = $parser->getHeader('cc')) {
            $this->originalCc = $resend ? $parser->getHeader('X-MailFlusher-Original-Cc') : $originalCc;
        }

        if ($originalTo = $parser->getHeader('to')) {
            $this->originalTo = $resend ? $parser->getHeader('X-MailFlusher-Original-To') : $originalTo;
        }
        ...
        if ($resend) {
            $this->originalFromHeader = base64_encode($parser->getHeader('X-MailFlusher-Original-From-Header'));
            $this->originalEnvelopeFrom = $parser->getHeader('X-MailFlusher-Original-Envelope-From');
            $this->originalReplyToHeader = base64_encode($parser->getHeader('X-MailFlusher-Original-Reply-To-Header'));
        } else {
        ...
        $this->authenticationResults = $parser->getHeader('X-MailFlusher-Authentication-Results');
        $this->receivedHeaders = $parser->getRawHeader('Received');

        $this->isSpam = $parser->getHeader('X-MailFlusher-Spam') === 'Yes';
        ...
        if ($parser->getParts()[1]['content-type'] === 'multipart/encrypted') {
            ...
            if ($isReplyOrSend && config('mailflusher.signing_key_fingerprint')) {

                // Check if encrypted with mailflusher.com public key and needs decrypting
    ...
    private function attemptToDecrypt($part)
    {
        try {
            ...
            $gnupg->adddecryptkey(config('mailflusher.signing_key_fingerprint'), null);
