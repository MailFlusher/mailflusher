...
use App\Notifications\Email\GpgKeyExpired;
use Exception;
...
class CustomMailer extends Mailer
{
    ...
    public function send($view, array $data = [], $callback = null)
    {
        ...
        if (isset($data['fingerprint']) && $data['fingerprint']) {
            ...
            try {
                $encrypter = new OpenPGPEncrypter(config('mailflusher.signing_key_fingerprint'), $data['fingerprint'], '~/.gnupg', $recipient->protected_headers);
        ...
        if (isset($data['needsDkimSignature']) && $data['needsDkimSignature'] && ! is_null(config('mailflusher.dkim_signing_key'))) {
            $dkimSigner = new DkimSigner(config('mailflusher.dkim_signing_key'), $data['aliasDomain'], config('mailflusher.dkim_selector'));
            ...
            $options = (new DkimOptions)->headersToIgnore([
                ...
                'MIME-Version',
                'Alias-To',
                'X-MailFlusher-Authentication-Results',
                'X-MailFlusher-Original-Sender',
                'X-MailFlusher-Original-Envelope-From',
                'X-MailFlusher-Original-From-Header',
                'X-MailFlusher-Original-To',
                'In-Reply-To',
                'References',
                'From',
                'To',
        ...
        if ($this->shouldSendMessage($symfonyMessage, $data)) {
            ...
            if (isset($data['emailType']) && in_array($data['emailType'], ['F', 'R', 'S'])) {
                ...
                $symfonyMessage->returnPath($verpLocalPart.'@'.config('mailflusher.domain'));
            }
            ...
            try {
                ...
                if ($user = User::find($userId)) {
                    ...
                    $failedDelivery = $user->failedDeliveries()->create([
                        ...
                        'sender' => $symfonyMessage->getHeaders()->get('X-MailFlusher-Original-Sender')?->getValue(),
    ...
    {
        $hmac = hash_hmac('sha3-224', $id, config('mailflusher.secret'));
