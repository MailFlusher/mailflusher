...
try {
    $repository = RepositoryBuilder::createWithDefaultAdapters()
        ->allowList([
            ...
            'ACTION_DEFER',
            'ACTION_DEFER_NEW',
            'MAILFLUSHER_ALL_DOMAINS',
            'MAILFLUSHER_SECRET',
            'MAILFLUSHER_ADMIN_USERNAME',
        ])
        ->make();
    ...
    $allDomains = explode(',', $_ENV['MAILFLUSHER_ALL_DOMAINS'] ?? '');

    $adminUsername = $_ENV['MAILFLUSHER_ADMIN_USERNAME'] ?? null;
    ...
    WHEN aliases.expires_at IS NOT NULL AND aliases.expires_at <= NOW() AND aliases.on_expiry = "bounce" THEN ?
    WHEN aliases.expires_at IS NOT NULL AND aliases.expires_at <= NOW() THEN ?
    WHEN aliases.max_emails IS NOT NULL AND aliases.emails_forwarded >= aliases.max_emails AND aliases.on_expiry = "bounce" THEN ?
    WHEN aliases.max_emails IS NOT NULL AND aliases.emails_forwarded >= aliases.max_emails THEN ?
    ...
    if ($noAliasExists && $aliasHasSharedDomain) {
        // If admin username is set and the domain matches the primary domain, allow catch-all for admin
        if ($adminUsername && $aliasDomain === ($allDomains[0] ?? null)) {
            $adminCatchAll = Database::table('usernames')
                ->leftJoin('users', 'usernames.user_id', '=', 'users.id')
                ->where('usernames.username', $adminUsername)
                ->where('usernames.catch_all', true)
                ->where('usernames.active', true)
                ->whereRaw('(users.reject_until IS NULL OR users.reject_until <= NOW())')
                ->whereRaw('(users.defer_until IS NULL OR users.defer_until <= NOW())')
                ->first();
...
function getIdFromVerp($verpLocalPart, $verpEmail)
{
    ...
    $expectedSignature = substr(hash_hmac('sha3-224', $id, $_ENV['MAILFLUSHER_SECRET'] ?? ''), 0, 8);
